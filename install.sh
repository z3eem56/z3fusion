...
#
# Copies:
#   skills/z3fusion        -> $CLAUDE_DIR/skills/z3fusion
#   skills/z3fusion-plan   -> $CLAUDE_DIR/skills/z3fusion-plan
...
rm -rf "$CLAUDE_DIR/skills/z3fusion"
cp -R "$HERE/skills/z3fusion" "$CLAUDE_DIR/skills/z3fusion"
rm -rf "$CLAUDE_DIR/skills/z3fusion-plan"
cp -R "$HERE/skills/z3fusion-plan" "$CLAUDE_DIR/skills/z3fusion-plan"
cp "$HERE/commands/"*.md "$CLAUDE_DIR/commands/"
chmod +x "$CLAUDE_DIR/skills/z3fusion/scripts/"*.sh
...
echo "    skills   : $CLAUDE_DIR/skills/z3fusion , $CLAUDE_DIR/skills/z3fusion-plan"
echo "    commands : /z3fusion  /z3fusion-claude  /z3fusion-gpt5.6  /z3fusion-gemini  /z3fusion-3  /z3fusion-plan"
echo

# Report which chains are usable on this machine.
have() { command -v "$1" >/dev/null 2>&1; }
echo "Panel availability here:"
echo "  claude-claude                  : ready (two independent in-session Claude panelist runs, judged by the"
echo "                                 orchestrating session — no external CLI)"
if have codex; then
  echo "  claude-gpt5.6               : ready (codex found: $(codex --version 2>/dev/null | head -1))"
else
  echo "  claude-gpt5.6               : needs the 'codex' CLI (install + log in for GPT-5.6 Sol)"
fi
if have agy; then
  echo "  claude-gpt5.6-gemini3.1pro  : ready (agy found: $(agy --version 2>/dev/null | head -1))"
else
  echo "  claude-gpt5.6-gemini3.1pro  : needs the 'agy' CLI (Antigravity; install + seed its keyring for Gemini 3.1 Pro)"
fi
echo
...
echo
echo "Register further custom local/remote runners in ~/.claude/z3fusion-runners.json"
echo "(see \$CLAUDE_DIR/skills/z3fusion/scripts/providers.sh for its exact shape)."
echo
echo "/z3fusion-plan (OMC-integrated iterative planning):"
echo "  - INTERACTIVE (you type /z3fusion-plan): runs an OMC interview first (auto-chains 'omc-plan'), then"
echo "    deepens with the 3-round claude-gpt5.6 panel and writes a concise plan to .omc/plans/."
...
echo
if [ -f "$CLAUDE_DIR/hooks/z3fusion-plan-nudge.sh" ]; then
  echo "Optional backstop hook installed (NOT enabled): $CLAUDE_DIR/hooks/z3fusion-plan-nudge.sh"
  ...
  echo "      \"command\":\"bash $CLAUDE_DIR/hooks/z3fusion-plan-nudge.sh\"}]}]}}"
  echo "  It nudges you to /z3fusion-plan --no-interview before delegating a non-trivial implementation task."
  echo
fi
